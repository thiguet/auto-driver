...
import {Car as CarRepo} from "../repository/Car";
...
export class CarController {
  ...
  @Get("/:id")
  @Returns(200, Car)
  public async get(@PathParams("id") id: number): Promise<Car> {
  ...
  public async update(@PathParams("id") id: number, @BodyParams("car") car: Car): Promise<Car> {
    ...
    return await (await this.carRepo.update(id, {...car, id})).raw[0];
  }

  @Delete("/:id")
  @Returns(200, Car)
  async delete(@PathParams("id") id: string): Promise<Car> {
    await this.carRepo.findOneOrFail(id);
